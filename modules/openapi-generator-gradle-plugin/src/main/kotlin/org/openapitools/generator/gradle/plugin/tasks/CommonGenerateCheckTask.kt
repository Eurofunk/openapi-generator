...
/**
 * A task with common settings and code for code generation and checking for changes.
 * Not to be executed individually.
 *
 * @author Jim Schubert
 */
@Suppress("UnstableApiUsage")
@CacheableTask
open class CommonGenerateCheckTask : DefaultTask() {
    /**
    ...
    fun processConfig(configurator: CodegenConfigurator) {
    ...
    fun processAdditionalProperties(clientOptInput: ClientOptInput) {
    ...
    fun selectCodegen(dryRun: Boolean): Generator? {
        var genName = "default"
        codegenName.ifNotEmpty { generator ->
            genName = generator
        }

        var selectedCodegen: Generator = DefaultGenerator(dryRun)
